// features/calculators/components/CalculatorOffPlan.jsx
import React, { useState } from 'react';
import Button from '../../../shared/components/ui/Button';
import { computeOffPlan } from '../utils/calc.offplan';
import { saveScenario } from '../../../services/clients.api';
export default function CalculatorOffPlan({ clientId }){
  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-2 gap-2">
        <input className="border rounded px-2 py-1" placeholder="Sinal" type="number" onChange={e=>setI({...i,deposit:+e.target.value})}/>
        <input className="border rounded px-2 py-1" placeholder="Prestações" type="number" onChange={e=>setI({...i,installments:+e.target.value})}/>
      </div>
